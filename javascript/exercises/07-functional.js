...
const mapNameFamily = (characters) => {
  ...
  let result = characters.map((item) => `${item.name}, of house ${item.house}`);
...
const filterFamily = (characters, house) => {
  ...
  let result = characters.filter((item) => item.house === house);
...
const reduceHouses = (characters) => {
  ...
  // const reducer = (acc, current) => acc + current.id;
